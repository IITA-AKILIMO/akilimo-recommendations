...
#* Vector of with recommendation texts to display.
#* Function with all inputs required by the DST app, and calling use case-specific wrapper functions to calculated recommendations on IC, FR, PP and SP.
...
#* @param SSPBagWt = 50
#* @param NPK112221available = FALSE
#* @param NPK112221CostperBag = NA
#* @param NPK112221BagWt = 50
#* @param NPK251010available = cFALSE
#* @param NPK251010CostperBag = NA
#* @param NPK251010BagWt = 50
#* @param NPK152020available = FALSE
#* @param NPK152020CostperBag = NA
#* @param NPK152020BagWt = 50
#* @param NPK23105available = FALSE
#* @param NPK23105CostperBag = NA
#* @param NPK23105BagWt = 50
#* @param NPK123017available = FALSE
#* @param NPK123017CostperBag = NA
#* @param NPK123017BagWt = 50
#* @param FOMIIMBURAavailable = FALSE
#* @param FOMIIMBURACostperBag = NA
#* @param FOMIIMBURABagWt = 50
#* @param FOMIBAGARAavailable = FALSE
#* @param FOMIBAGARACostperBag = NA
#* @param FOMIBAGARABagWt = 50
#* @param FOMITOTAHAZAavailable = FALSE
#* @param FOMITOTAHAZACostperBag = NA
#* @param FOMITOTAHAZABagWt = 50
...
#* @serializer json
#* @post /computes
getRecommendation <- function(country = c("NG", "TZ", "RW", "GH"),
                              lat,
                              lon,
                              area,
...
{
  print(paste("Starting plumbR version 3 script. processing", country))
  ...
  if (country == "BI") {
    country <- "BU"
    print(paste("Switching to non standard country code for Burundi", country))
  }
  #set default values for the new fertilizer
  ...
  if (FR) {
    print("Processing FR section now")
      ...
      # Generate FR recommendations
      res[["FR"]] <- getFRrecommendations(
        lat = lat,
        lon = lon,
        pd = pd,
        pw = pw,
    ...
    cat("Finshed processing")


    # Check for specific messages and set FRrecom and recText accordingly
    ...
    if (all(res$FR == no_recommendation_msg)) {
      ...
      if (res[["FR"]]$rec$NR > 0) {
        ...
        # Handle cases with no recommendations
        FRrecom <- FALSE

        switch(
          country,
          "NG" = { recText[["FR"]] <- frnotrec_ng },
          "GH" = { recText[["FR"]] <- frnotrec_ng },  # Assuming frnotrec_ng is used for GH as well
          "RW" = { recText[["FR"]] <- frnotrec_rw },
          "TZ" = { recText[["FR"]] <- frnotrec_tz },
          { recText[["FR"]] <- "No recommendation available" }  # Default case
        )
      }
    }
  }
  ...
  if (IC && country == "NG") {
    ...
    if (nrow(res$IC[[2]]) > 0) {
      ...
      # Update fertilizer advice table
      fertilizerAdviseTable(
        FR = FALSE,
        IC = TRUE,
        country = "NG",
  ...
  if (IC && country == "TZ") {
    ...
    if (nrow(res$IC[[2]]) > 0) {
      ...
      # Save recommendation text to CSV file
  ...
  if (PP) {
    ...
    # Generate text for PP recommendations
    recText[["PP"]] <- getPPrecText(ds = res$PP, country = country)

    # Save PP recommendations and text to CSV files
  ...
  if (SPP || SPH) {
    if (PD_window == 0 && HD_window == 0) {
      ...
      # Generate schedule planting recommendations
      print("Processing SP")
      ...
      if (!is.data.frame(res[["SP"]])) {
        ...
        # Save recommendations and text to CSV file
  ...
  # Ensure default values for recommendation variables
  FRrecom <- ifelse(is.null(FRrecom), FALSE, FRrecom)
  ICrecom <- ifelse(is.null(ICrecom), FALSE, ICrecom)
  SPrecom <- ifelse(is.null(SPrecom), FALSE, SPrecom)

  # Determine if PP recommendations are present
  PPrecom <- !is.null(res[["PP"]])

  # Determine if SP is TRUE based on SPP or SPH
  SP <- SPP || SPH
  ...
  if (email) {
    ...
    tryCatch({
      ...
      print(paste("Warning occurred while sending email:", w))
    }, error = function(e) {
      print(paste("Error occurred while sending email:", e))
    })
  }

  # Print status messages
  #print(paste("Returning recommendations back to AKILIMO API for country", country))
  #print(paste("Data value of res is:", res))
  #print("---------------------------------------------")
  #print(paste("Data value of recText is:", recText))

  # Return results
  response <- list(res = res, recText = recText)
  #print(paste("Response is :--> ", response))
  #return(response)
...
function(req, res) {
  tryCatch({
    ...
    PD <- body$PD
    HD <- body$HD
    PD_window <- body$PD_window
    HD_window <- body$HD_window
    fallowType <- body$fallowType
