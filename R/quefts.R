...
QUEFTS <- function(QID, rec, HI=0.52) {
	...
	do_quefts <- function(supply) {
		...
		final_yield <- function(Uptake_Yield) {
			...
			# nutrient limited yields cannot exceed the maximum possible yield = WLY
			...
			#Final estimate #YEc <- 
			mean(c(YNPc, YNKc, YPNc, YPKc, YKNc, YKPc))
		  })
		}

		actual_uptake <- function(ds_supply) {
		...
		actualUptake <- cbind(supply, actual_uptake(supply))
		minmax_Yield <- cbind(actualUptake, max_min_yields(actualUptake))
	 ## yield at zero input
		final_yield(minmax_Yield)
	}
