...
| `R/sms_email.R` | PDF generation orchestration; email dispatch (smtp/mailtrap/mailgun); SMS |
| `R/misc.R` | `tr(key, lang, ...)` translation helper, `get_currency()`, `getRFY()`/`getRDY()` |

### PDF pipeline
PDF reports are generated by [WeasyPrint](https://weasyprint.org) (Python CLI). Install with `pip install weasyprint`. No browser or Java required. See [docs/PDF-GENERATION-PLAN.md](docs/PDF-GENERATION-PLAN.md) for full details.
...
PDF section headings and UI labels (not shared with SMS/email) are stored in `.PDF_LABELS` in `R/html_helpers.R` and looked up via `html_label(key, lang)`. See [docs/PDF-GENERATION-PLAN.md §10](docs/PDF-GENERATION-PLAN.md#10-language-support) for the rationale.
