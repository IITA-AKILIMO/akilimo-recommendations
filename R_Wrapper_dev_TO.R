...
getRecommendation <- function(country = c("NG", "TZ", "RW"),
                               lat,
                               lon,
                               area,
  ...
  #setwd("D:/akilimo/akilimo_recommendation")
...
#rw
	...
	recloc_ng <- gsub(pattern = "\"",replacement = "",TRNS$recloc[1]) ;	recloc_tz <- gsub(pattern = "\"",replacement = "",TRNS$recloc[2]) ;  recloc_rw <- gsub(pattern = "\"",replacement = "",TRNS$recloc[3]) ; frnotrec_ng <- gsub(pattern = "\"",replacement = "",TRNS$frnotrec[1]) ;
 	frnotrec_tz <- gsub(pattern = "\"",replacement = "",TRNS$frnotrec[2]) ; frnotrec_rw <- gsub(pattern = "\"",replacement = "",TRNS$frnotrec[3]) ; spinfo_ng <- gsub(pattern = "\"",replacement = "",TRNS$spinfo[1]) ; spinfo_tz <- gsub(pattern = "\"",replacement = "",TRNS$spinfo[2])
	spinfo_rw <- gsub(pattern = "\"",replacement = "",TRNS$spinfo[3])
	...
	if(FR == TRUE)   {res[["FR"]] <- getFRrecommendations(lat = lat,
                                          	...
                                          	if(all(res$FR == "We do not have fertilizer recommendation for your location because your location is out of the recommendation domain AKILIMO is currently serving.") & country == "NG"){
                                          	  ...
                                          	  FRrecom <- FALSE
                                          	  recText[["FR"]] <- res$FR
                                          	}else if (all(res$FR == "KINYARWANDA") & country == "RW"){
                                          	  FRrecom <- FALSE
                                          	  recText[["FR"]] <- res$FR
                                          	}else{
                                          	  if(res[["FR"]]$rec$NR > 0){
	...
	if(IC == TRUE & country == "NG")                        {res[["IC"]] <- getICrecommendations(areaHa = areaHa,
          	                                                      ...
          	                                                      if (nrow(res$IC[[2]]) > 0){
          	                                                        ...
          	                                                        fertilizerAdviseTable(FR=FALSE, IC=TRUE, country = c("NG", "RW") , areaUnits=areaUnits)
          	                                                      }else{
          	                                                        ICrecom <- FALSE
