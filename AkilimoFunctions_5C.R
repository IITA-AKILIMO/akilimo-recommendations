...
getSPrecommendations <- function(areaHa,
  ...
  SoilData_fcy1 <- readRDS("/home/akilimo/projects/akilimo_recommendation/SoilData_4Country.RDS")
  ...
  TRNS <- read.csv("/home/akilimo/projects/akilimo_recommendation/translations_TEST.csv", stringsAsFactors = FALSE)
...
#' it reads the NOT data and add teh FCY to train a random forest model and then use the model to predict INS for the user GPS
...
getFRrecommendations <- function(lat, lon, pd, pw, HD, had, maxInv, fertilizers, rootUP, areaHa, country, FCY, riskAtt) {
  ...
  if (country == "NG") {
    ...
    WLY_365 <- readRDS("/home/akilimo/projects/akilimo_recommendation/Burundi_WLY_LINTUL.RDS")
    WLY_365$pl_Date <- WLY_365$plantingDate
    WLY_365$PlweekNr <- WLY_365$weekNr
    colnames(WLY_365) <- gsub("WLY_", "", colnames(WLY_365))
    ...
    if(country %in% c("NG", "TZ")){
      ...
      fcyy <- ifelse(FCY < 7.5, "FCY1",
                     ifelse(FCY >= 7.5 & FCY < 15, "FCY2",
                            ifelse(FCY >= 15 & FCY < 22.5, "FCY3",
                                   ifelse(FCY >= 22.5 & FCY < 30, "FCY4", "FCY5"))))
      SoilData <- readRDS(paste("/home/akilimo/projects/akilimo_recommendation/BU_", fcyy, "_soilNPK.RDS", sep=""))
      SoilData$location <- paste(SoilData$lat, SoilData$lon, sep="_")
      SoilData <- SoilData[SoilData$location == wlydata$location, ]
      SoilData$Zone <- "BU"
      SoilData$rec_N <- 0.5
...
getFRrecText <- function(ds, country, fertilizers, rootUP) {
  ...
  if (is.null(rec)) {
    ...
    #trans
    recom <- if (country == "NG" | country == "GH" | country == "BU") {
      paste(norecom_ng)
    }else if (country == "TZ"){
      paste(norecom_tz)
    }else{
