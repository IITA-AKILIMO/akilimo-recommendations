...
#* @json
#* @post /api/v2/dst/fertilizer-recommendation
#* @put /api/v2/dst/fertilizer-recommendation
#* @get /api/v2/dst/fertilizer-recommendation
...
newFert5name = c(TRUE, FALSE), newFert5N_cont = NA, newFert5P2O5 = NA, newFert5K2O = NA, newFert5CostperBag = NA, newFert5BagWt = NA)
{
    ...
    # IC = FR = PP = SPP = SPH = TRUE
    # intercrop <- FALSE
    intercrop = IC
    SMS = email = TRUE; userPhoneCC = 254; userPhoneNr = 702974480; userName = "Meklit"; userEmail = "akilimo@gmail.com"; userField = "Runda field"
    ...
    if(FR == TRUE & intercrop == FALSE)   {res[["FR"]] <- getFRrecommendations(lat = lat,
    lon = lon,
    pd = pd,
    pw = pw,
    HD=HD,
    ...
    FCY=FCY
        ...
        if(all(res[["FR"]] != "We do not have fertilizer recommendation for your location.")){
    ...
    riskAtt = riskAtt)
        ...
        IC_MarkdownText(rr = res$IC, fertilizers=fertilizers, userName=userName, country=country, maizeUP = maizeUP,maizePD= maizePD,
        userPhoneNr=userPhoneNr, userField=userField, area=area, areaUnits=areaUnits,maizeUW = maizeUW,
        PD =PD, HD=HD, email=email, lat=lat, lon=lon, userPhoneCC=userPhoneCC,
        rootUP = rootUP, cassPD = cassPD, maxInv = maxInv, CMP=CMP)
    ...
    rootUP_p2 = rootUP_p2)
        if(!is.data.frame(res[["SP"]])){
            ...
            write.csv(res$SP, 'SP_rec.csv', row.names = FALSE)
    ...
    # return(recText)
    print("Returning computed values now.....")
