...
print(paste("Running email generation",FR, IC, PP, SP, FRrecom, ICrecom))
  if (FR == TRUE &
    ...
    FRrecom == TRUE &
    country == "NG") {
    if (file.exists("/home/akilimo/projects/akilimo_recommendation/fertilizer_advice_VFT.pdf"))
  ...
  if (!is.null(listofPDFs)) {
    send.mail(from = "AKILIMO@cgiar.org",
              ...
              smtp = list(host.name = "smtp.office365.com", port = 587,
                          user.name = "AKILIMO@cgiar.org", passwd = "pktjltndtrsdvcml", tls = TRUE))
...
fertilizerAdviseTable <- function(FR, IC, country, areaUnits) {
  ...
  acairm$currency  <- ifelse(acairm$country == "NG", "NGN", ifelse(acairm$country == "RW", "RWF",  "TZS"))
...
getFRrecText <- function(ds, country, fertilizers, rootUP) {
  ...
  if (is.null(rec)) {
    ...
    if (rec$TC == 0) {
      ...
      currency <- ifelse(country == "NG", "NGN", ifelse(country == "RW", "RWF",  "TZS"))
...
getPPrecText <- function(ds, country = c("NG", "TZ")) {
  ...
  if (ds[1,]$CP) {
    ...
    if (ds[1,]$ploughing & !ds[1,]$ridging) { recT <- if (country == "NG") {
           ...
           paste0(werec_tz, noridg_tz, ds[1,]$method_ploughing,  "\n")
        }else{
    ...
    if (!ds[1,]$ploughing & ds[1,]$ridging) { recT <- if (country == "NG") {
          paste0(plodir_ng, ds[1,]$method_ridging, ridg_ng, "\n")
            ...
            paste0(plodir_tz, ridg_tz, ds[1,]$method_ridging,  "\n")
           }else{
