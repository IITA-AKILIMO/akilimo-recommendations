...
getPPrecommendations <- function(areaHa,
  ...
  ds$CP <- ifelse(ds$ploughing == ploughing &
                    ds$method_ploughing == method_ploughing &
                    ds$ridging == ridging &
                    ds$method_ridging == method_ridging, TRUE, FALSE)
...
#                          K_cont = c(0, 0.6, 0, 0.125, 0.083, 0),
...
getSPrecommendations <- function(areaHa,
  ...
  # if(country == "NG"){
  #   WLY_15M <- readRDS("/home/akilimo/projects/akilimo_recommendation/Nigeria_WLY_LINTUL_2020.RDS")
  # }else{
  #   WLY_15M <- readRDS("/home/akilimo/projects/akilimo_recommendation/Tanzania_WLY_LINTUL_2020.RDS")  }
  #
  # WLY_15Month <- gather(WLY_15M, daysOnField, water_limited_yield, "235":"452")
  # WLY_15Month$harvestDay <- WLY_15Month$pl_Date  +  as.numeric(as.character(WLY_15Month$daysOnField))
  # WLY_15Month$zone <- country
  # WLY_15Month <- WLY_15Month[, c("lat", "long", "water_limited_yield","location", "pl_Date","zone", "harvestDay", "daysOnField", "PlweekNr")]
  # saveRDS(WLY_15Month, "Tanzania_WLY_LINTUL_2020_Server.RDS")

  if (country == "NG") {
    WLY_15M <- readRDS("/home/akilimo/projects/akilimo_recommendation/Nigeria_WLY_LINTUL_2020_Server.RDS")
  } else if (country == "TZ"){
    WLY_15M <- readRDS("/home/akilimo/projects/akilimo_recommendation/Tanzania_WLY_LINTUL_2020_Server.RDS")
  } else if (country == "GH"){
    WLY_15M <- readRDS("/home/akilimo/projects/akilimo_recommendation/Ghana_WLY_LINTUL_SP.RDS")
  }
  ...
  #pick up the upper (water-limited) and lower (control) dry root yields
  # if(country == "NG"){
  #   flp <-paste0("/home/akilimo/projects/akilimo_recommendation/inputDST_SPHS_perpixel/E", lonr, "N", latr, ".csv")
  # }else{
  #   flp <-paste0("/home/akilimo/projects/akilimo_recommendation/inputDST_SPHS_perpixel_TZ/E", lonr, "N", latr, ".csv")
  # }
  #

  # if(latr > -5 & month(PD) %in% c(2, 6, 7)){
  #   return("Please change your planting month, it is not recommended to plant cassava in the Lake zone in Feb, Jun and July .")
  # }
  #
  # if(latr < -5 & latr > -9 & month(PD) %in% c(5,6,7,8)){
  #   return("Please change your planting month, it is not recommended to plant cassava in the Eastern zone in May, Jun, Jul, Aug.")
  #
  # }
  #
  # if(latr < -9 & month(PD) %in% c(3,4,5,6,7)){
  #   return("Please change your planting month, it is not recommended to plant cassava in the Southern zone in Mar, Apr, May, Jun, Jul and Aug .")
  #
  # }
  ...
  if (is.null(WLY_CY)) {
    ...
    #yld <- read.csv(flp)
    # yld <- unique(yld)
    # WLY_CY$HD <- WLY_CY$harvestDay - WLY_CY$pl_Date
    # WLY_CY$HD <- ifelse(WLY_CY$HD > 365, WLY_CY$HD-365, WLY_CY$HD)
...
##getFRrecommendations_withprecalc_CY = using precalcualted CY
getFRrecommendations_withprecalc_CY <- function(lat, lon, PD, maxInv, fertilizers, wlyd, SoilData, rootUP, areaHa, country,riskAtt) {


  WLYData <- wlyd[wlyd$pl_Date == PD,]
  ...
  if (fert_optim$NR <= 0) { ## all fertilizer recom == 0
    ...
    fert_optim <- cbind(fertinfo, onlyFert)
    return(fert_optim)
  }else {
    ...
    if (nrow(onlyFert2) == 0) { ## if all fertilizer recom < 25 kg/ha all will be set to 0
      ...
      Reset_fert_Cont <- Rerun_25kgKa_try(rootUP = rootUP, rdd = fert_optim2, fertilizer = fertilizer, QID = SoilData, onlyFert = onlyFert2,
                                          country = country, WLY = water_limited_yield, DCY = DCY, areaHa = areaHa)
      if (Reset_fert_Cont$NR <= 0) { ## after rerunning after avoiding <25KG/ha fertilizers, if NR <=0
        ...
        Reset_fert_Cont <- cbind(Reset_fert_Cont, onlyFert)
...
fertilizerFunc <- function(NPK201216available = TRUE, NPK201216CostperBag = NA, NPK201216BagWt = 50,
  ...
  if (country == "NG") {
    ...
    if (NPK171717CostperBag == 0) { NPK171717CostperBag <- 35650}else { NPK171717CostperBag <- as.numeric(NPK171717CostperBag) }#713 *50
     ...
     if (FOMIIMBURACostperBag == 0) { FOMIIMBURACostperBag <- 85390} else { FOMIIMBURACostperBag <- as.numeric(FOMIIMBURACostperBag) }
     if (FOMIBAGARACostperBag == 0) { FOMIBAGARACostperBag <- 77600}else { FOMIBAGARACostperBag <- as.numeric(FOMIBAGARACostperBag) }
     if (FOMITOTAHAZAavailable == 0) { FOMITOTAHAZAavailable <- 77000  }else { FOMITOTAHAZAavailable <- as.numeric(FOMITOTAHAZAavailable) }
   }
  ...
  FOMIIMBURAFert <- data.frame(type = 'FOMI_IMBURA', available = FOMIIMBURAavailable, N_cont = 0.20, P_cont = 0.052, K_cont = 0.216, costPerBag = FOMIIMBURACostperBag, bagWeight = FOMIIMBURABagWt)
  CANFert <- data.frame(type = 'CAN', available = CANavailable, N_cont = 0.27, P_cont = 0.00, K_cont = 0.00, costPerBag = CANCostperBag, bagWeight = CANBagWt) ## not correct value TODO check
  SSPFert <- data.frame(type = 'SSP', available = SSPavailable, N_cont = 0.00, P_cont = 0.15, K_cont = 0.00, costPerBag = SSPCostperBag, bagWeight = SSPBagWt) ## not correct value TODO check
...
getRFY <- function(HD,
  ...
  fd <- read.csv("/home/akilimo/projects/akilimo_recommendation/fd2.csv") #data.frame with day of the year (dayNr = [1..366]) and %DM (DMCont = [0..100], by country)
  # fd <- read.csv("fd2.csv")
...
getRDY <- function(HD, RFY, country) {
  ...
  d <- HD
  fd <- read.csv("/home/akilimo/projects/akilimo_recommendation/fd2.csv")
  #fd <- read.csv("fd2.csv")
...
print(paste("Running email generation",FR, IC, PP, SP, FRrecom, ICrecom))
  if (FR == TRUE &
    IC == FALSE &
    FRrecom == TRUE &
    country == "NG" ) {
    if (file.exists(paste("/home/akilimo/projects/akilimo_recommendation/fertilizer_advice_", userPhoneNr, ".pdf",sep="")))
  ...
  if (FR == TRUE &
    ...
    FRrecom == TRUE &
    country == "TZ") {
    #if (file.exists("/home/akilimo/projects/akilimo_recommendation/fertilizer_advice_swa.pdf"))
  ...
  if (IC == TRUE &
    ...
    country == "NG" &
    ICrecom == TRUE) {
    if (file.exists("/home/akilimo/projects/akilimo_recommendation/intercrop_advice_VFT.pdf"))
  ...
  if (SP == TRUE & SPrecom == TRUE & country == "TZ") {
    if (file.exists("/home/akilimo/projects/akilimo_recommendation/SP_advice_swa.pdf"))
      webshot::rmdshot('SP_markdown_swa.Rmd', file = paste0('SP_advice_swa_', userPhoneNr, ".pdf"), delay = 3)
    if (file.exists("spgg.png")) file.remove("spgg.png")
  }

  listofPDFs <- NULL
  if (FR == TRUE & FRrecom == TRUE & country == "NG") { listofPDFs <- c(listofPDFs, paste("/home/akilimo/projects/akilimo_recommendation/fertilizer_advice_", userPhoneNr, ".pdf", sep = "")) }
  if (FR == TRUE & FRrecom == TRUE & country == "GH") { listofPDFs <- c(listofPDFs, paste("/home/akilimo/projects/akilimo_recommendation/fertilizer_advice_", userPhoneNr, ".pdf", sep = "")) }
  
  
  if (FR == TRUE & FRrecom == TRUE & country == "TZ") { listofPDFs <- c(listofPDFs, paste("/home/akilimo/projects/akilimo_recommendation/fertilizer_advice_swa_", userPhoneNr, ".pdf", sep = "")) }

  if (PP == TRUE & PPrecom == TRUE & country == "NG") { listofPDFs <- c(listofPDFs, paste("/home/akilimo/projects/akilimo_recommendation/PP_advice_", userPhoneNr, ".pdf", sep = "")) }

  if (PP == TRUE & PPrecom == TRUE & country == "TZ") { listofPDFs <- c(listofPDFs, paste("/home/akilimo/projects/akilimo_recommendation/PP_advice_swa_", userPhoneNr, ".pdf", sep = "")) }

  if (SP == TRUE & SPrecom == TRUE & country == "NG") { listofPDFs <- c(listofPDFs, paste("/home/akilimo/projects/akilimo_recommendation/SP_advice_", userPhoneNr, ".pdf", sep = "")) }
  if (SP == TRUE & SPrecom == TRUE & country == "GH") { listofPDFs <- c(listofPDFs, paste("/home/akilimo/projects/akilimo_recommendation/SP_advice_", userPhoneNr, ".pdf", sep = "")) }
  
  if (SP == TRUE & SPrecom == TRUE & country == "TZ") { listofPDFs <- c(listofPDFs, paste("/home/akilimo/projects/akilimo_recommendation/SP_advice_swa_", userPhoneNr, ".pdf", sep = "")) }

  if (IC == TRUE & country == "NG" & ICrecom == TRUE) { listofPDFs <- c(listofPDFs, paste("/home/akilimo/projects/akilimo_recommendation/intercrop_advice_", userPhoneNr, ".pdf", sep = "")) }

  if (IC == TRUE & country == "TZ" & ICrecom == TRUE) { listofPDFs <- c(listofPDFs, paste("/home/akilimo/projects/akilimo_recommendation/CIS__advice", userPhoneNr, ".pdf", sep = "")) }

  # if(FR == TRUE){listofPDFs <- c(listofPDFs, "D:/ACAI_Wrapper/cloud_compute/fertilizer_advice.pdf")}
  # if(PP == TRUE){listofPDFs <- c(listofPDFs, "D:/ACAI_Wrapper/cloud_compute/PP_advice.pdf")}
  # if(SP == TRUE){listofPDFs <- c(listofPDFs, "D:/ACAI_Wrapper/cloud_compute/SP_advice.pdf")}
  # if(IC == TRUE & country == "NG") {listofPDFs <- c(listofPDFs, "D:/ACAI_Wrapper/cloud_compute/intercrop_advice.pdf")}
  #
  ...
  #           smtp = list(host.name = "smtp.gmail.com", port = 587, user.name = "acai.akilimo@gmail.com", passwd = "akilimo101", tls = T),
  ...
  library(mailR)
  sender <- "acai.akilimo@gmail.com"
  recipients <- c("barsamms@gmail.com")
  send.mail(from = sender,
            to = recipients,
            subject = "Test mail from Rstudio",
            body = "Test email body",
            #attach.files = "intercrop_advice_702974480.pdf",
            smtp = list(host.name = "smtp.gmail.com", port = 587, 
                        user.name = "acai.akilimo@gmail.com",            
                        passwd = "akilimo101", tls = TRUE),
            authenticate = TRUE,
            send = TRUE) 
...
#TO included code for NPK1520202 to read from orange bags then set grey bags other
      ...
      if (dat$bag == 0.5) {
        dat$rep <- sprintf(paste('![](/home/akilimo/projects/akilimo_recommendation/net/', fertColCode, '/half.png)', sep = ""))
      }else if (dat$bag == 1) {
        dat$rep <- sprintf(paste('![](/home/akilimo/projects/akilimo_recommendation/net/', fertColCode, '/1.png)', sep = ""))
      }else if (dat$bag == 1.5) {
        dat$rep <- sprintf(paste('![](/home/akilimo/projects/akilimo_recommendation/net/', fertColCode, '/1_5.png)', sep = ""))
      }else if (dat$bag == 2) {
        dat$rep <- sprintf(paste('![](/home/akilimo/projects/akilimo_recommendation/net/', fertColCode, '/2.png)', sep = ""))
      }else if (dat$bag == 2.5) {
        dat$rep <- sprintf(paste('![](/home/akilimo/projects/akilimo_recommendation/net/', fertColCode, '/2_5.png)', sep = ""))
      }else if (dat$bag == 3) {
        dat$rep <- sprintf(paste('![](/home/akilimo/projects/akilimo_recommendation/net/', fertColCode, '/3.png)', sep = ""))
      }else if (dat$bag == 3.5) {
        dat$rep <- sprintf(paste('![](/home/akilimo/projects/akilimo_recommendation/net/', fertColCode, '/3_5.png)', sep = ""))
      }else if (dat$bag == 4) {
        dat$rep <- sprintf(paste('![](/home/akilimo/projects/akilimo_recommendation/net/', fertColCode, '/4.png)', sep = ""))
      }else if (dat$bag == 4.5) {
        dat$rep <- sprintf(paste('![](/home/akilimo/projects/akilimo_recommendation/net/', fertColCode, '/4_5.png)', sep = ""))
      }else if (dat$bag == 5) {
        dat$rep <- sprintf(paste('![](/home/akilimo/projects/akilimo_recommendation/net/', fertColCode, '/5.png)', sep = ""))
      }else if (dat$bag == 5.5) {
        dat$rep <- sprintf(paste('![](/home/akilimo/projects/akilimo_recommendation/net/', fertColCode, '/5_5.png)', sep = ""))
      }else if (dat$bag == 6) {
        dat$rep <- sprintf(paste('![](/home/akilimo/projects/akilimo_recommendation/net/', fertColCode, '/6.png)', sep = ""))
      }else if (dat$bag == 6.5) {
        dat$rep <- sprintf(paste('![](/home/akilimo/projects/akilimo_recommendation/net/', fertColCode, '/6_5.png)', sep = ""))
      }else if (dat$bag == 7) {
        dat$rep <- sprintf(paste('![](/home/akilimo/projects/akilimo_recommendation/net/', fertColCode, '/7.png)', sep = ""))
      }else if (dat$bag == 7.5) {
        dat$rep <- sprintf(paste('![](/home/akilimo/projects/akilimo_recommendation/net/', fertColCode, '/7_5.png)', sep = ""))
      }else if (dat$bag == 8) {
        dat$rep <- sprintf(paste('![](/home/akilimo/projects/akilimo_recommendation/net/', fertColCode, '/8.png)', sep = ""))
      }else if (dat$bag == 8.5) {
        dat$rep <- sprintf(paste('![](/home/akilimo/projects/akilimo_recommendation/net/', fertColCode, '/8_5.png)', sep = ""))
      }else if (dat$bag == 9) {
        dat$rep <- sprintf(paste('![](/home/akilimo/projects/akilimo_recommendation/net/', fertColCode, '/9.png)', sep = ""))
      }else if (dat$bag == 9.5) {
        dat$rep <- sprintf(paste('![](/home/akilimo/projects/akilimo_recommendation/net/', fertColCode, '/9_5.png)', sep = ""))
      }else if (dat$bag == 10) {
        dat$rep <- sprintf(paste('![](/home/akilimo/projects/akilimo_recommendation/net/', fertColCode, '/10.png)', sep = ""))
      }
  ...
  if (ratioFertCost == 1) {
    totalCostmoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture1.png)')
  } else if (ratioFertCost == 2) {
    totalCostmoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture2.png)')
  }else if (ratioFertCost == 3) {
    totalCostmoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture3.png)')
  }else if (ratioFertCost == 4) {
    totalCostmoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture4.png)')
  }else if (ratioFertCost == 5) {
    totalCostmoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture5.png)')
  }else if (ratioFertCost == 6) {
    totalCostmoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture6.png)')
  }else if (ratioFertCost == 7) {
    totalCostmoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture7.png)')
  }else if (ratioFertCost == 8) {
    totalCostmoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture8.png)')
  }else if (ratioFertCost == 9) {
    totalCostmoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture9.png)')
  }else if (ratioFertCost == 10) {
    totalCostmoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture10.png)')
  }
  write.csv(totalCostmoney, "totalCostmoney.csv", row.names = FALSE)

  if (ratioTotalSale == 1) {
    totalSalemoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture1.png)')
  } else if (ratioTotalSale == 2) {
    totalSalemoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture2.png)')
  }else if (ratioTotalSale == 3) {
    totalSalemoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture3.png)')
  }else if (ratioTotalSale == 4) {
    totalSalemoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture4.png)')
  }else if (ratioTotalSale == 5) {
    totalSalemoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture5.png)')
  }else if (ratioTotalSale == 6) {
    totalSalemoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture6.png)')
  }else if (ratioTotalSale == 7) {
    totalSalemoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture7.png)')
  }else if (ratioTotalSale == 8) {
    totalSalemoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture8.png)')
  }else if (ratioTotalSale == 9) {
    totalSalemoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture9.png)')
  }else if (ratioTotalSale == 10) {
    totalSalemoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture10.png)')
  }
  write.csv(totalSalemoney, "totalSalemoney.csv", row.names = FALSE)


  if (ratioRevenue == 1) {
    totalRevenuemoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture1.png)')
  } else if (ratioRevenue == 2) {
    totalRevenuemoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture2.png)')
  }else if (ratioRevenue == 3) {
    totalRevenuemoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture3.png)')
  }else if (ratioRevenue == 4) {
    totalRevenuemoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture4.png)')
  }else if (ratioRevenue == 5) {
    totalRevenuemoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture5.png)')
  }else if (ratioRevenue == 6) {
    totalRevenuemoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture6.png)')
  }else if (ratioRevenue == 7) {
    totalRevenuemoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture7.png)')
  }else if (ratioRevenue == 8) {
    totalRevenuemoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture8.png)')
  }else if (ratioRevenue == 9) {
    totalRevenuemoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture9.png)')
  }else if (ratioRevenue == 10) {
    totalRevenuemoney$moneypack <- sprintf('![](/home/akilimo/projects/akilimo_recommendation/net/cash/Picture10.png)')
  }
...
Rfmodel_Wrapper_obsolete <- function(ISRIC_SoilData = ISRIC_SoilData, FCY = FCY, country = country) {
  ...
  GIS_soilINS_modData2 <- read.csv("/home/akilimo/projects/akilimo_recommendation/NOT_GIS_CON.csv") ## NOT data used to develope the RF model
  ...
  NG_SoilData <- read.csv('/home/akilimo/projects/akilimo_recommendation/NG_SoilData.csv')
...
#' gets lat or lon and find the closest matching lat and lon in the WLY data, at 0.05 (5Km) resolution
...
#' to get ISiric data, fir RF model, get soil NPK and get CY for FCY at HD and PD
...
ISRIC_FRmodel_CY <- function(lat = lat, lon = lon, country = country, FCY = FCY, PD = PD, HD = HD) {
  ...
  Nigeria_WLY_365 <- readRDS("/home/akilimo/projects/akilimo_recommendation/NG_WLY_LINTUL.RDS")
...
getFRrecommendations <- function(lat, lon, pd, pw, HD, had, maxInv, fertilizers, rootUP, areaHa, country, FCY,riskAtt) {
  ...
  # lat2 <- getclosestcoor(ll=lat)
  # lon2 <- getclosestcoor(ll=lon)
  # lat2 <- round(floor(lat*10)/10 + ifelse(lat-(floor(lat*10)/10) < 0.05, 0.025, 0.075), digits=3)
  # lon2 <- round(floor(lon*10)/10 + ifelse(lon-(floor(lon*10)/10) < 0.05, 0.025, 0.075), digits=3)
  #
  latr <- as.factor(floor(lat * 10) / 10 + ifelse(lat - (floor(lat * 10) / 10) < 0.05, 0.025, 0.075))
  lonr <- as.factor(floor(lon * 10) / 10 + ifelse(lat - (floor(lon * 10) / 10) < 0.05, 0.025, 0.075))
  ...
  # if(country == "NG"){
  #   WLY_365 <- readRDS("/home/akilimo/projects/akilimo_recommendation/NG_WLY_LINTUL.RDS")
  # }else{
  #   WLY_365 <- readRDS("/home/akilimo/projects/akilimo_recommendation/TZ_WLY_LINTUL.RDS")
  # }

  if (country == "NG") {
    WLY_365 <- readRDS("/home/akilimo/projects/akilimo_recommendation/Nigeria_WLY_LINTUL_2020.RDS")
  }else if (country == "TZ") {
    WLY_365 <- readRDS("/home/akilimo/projects/akilimo_recommendation/Tanzania_WLY_LINTUL_2020.RDS")
  }else if (country == "RW") {
    WLY_365 <- readRDS("/home/akilimo/projects/akilimo_recommendation/Rwanda_WLY_LINTUL.RDS")
    WLY_365$pl_Date <- WLY_365$plantingDate
    WLY_365$PlweekNr <- WLY_365$weekNr
    colnames(WLY_365) <- gsub("WLY_", "", colnames(WLY_365))
  }else if(country == "GH"){
    WLY_365 <- readRDS("/home/akilimo/projects/akilimo_recommendation/Ghana_WLY_LINTUL.RDS")
    WLY_365$pl_Date <- WLY_365$plantingDate
    WLY_365$PlweekNr <- WLY_365$weekNr
    colnames(WLY_365) <- gsub("WLY_", "", colnames(WLY_365))
  }
  ...
  if (nrow(wlypd) == 0) {
    #return(list(rec=NULL, fertilizer_rates = NULL))
    #issuereturn
    ...
    wlydata$water_limited_yield <- wlypd[, colnames(wlypd) == HD2]
    # if(country == "TZ"){
    #   wlydata$water_limited_yield <- ifelse(wlydata$water_limited_yield > 20000, 20052, as.numeric(as.character(wlydata$water_limited_yield)))
    # }
    wlydata$zone <- country
    ...
    if(country %in% c("NG", "TZ")){
      ...
      SoilData <- readRDS(paste("/home/akilimo/projects/akilimo_recommendation/RW_", fcyy, "_soilNPK.RDS", sep=""))
      ...
      fcyy <- ifelse(FCY < 7.5, "FCY1",
                     ifelse(FCY >= 7.5 & FCY < 15, "FCY2",
                            ifelse(FCY >= 15 & FCY < 22.5, "FCY3",
                                   ifelse(FCY >= 22.5 & FCY < 30, "FCY4", "FCY5"))))
      SoilData <- readRDS(paste("/home/akilimo/projects/akilimo_recommendation/GH_", fcyy, "_soilNPK.RDS", sep=""))
      SoilData$location <- paste(SoilData$lat, SoilData$lon, sep="_")
      SoilData <- SoilData[SoilData$location == wlydata$location, ]
      SoilData$Zone <- "RW"
      SoilData$rec_N <- 0.5
    ...
    WLYData$weekNr <- pw
    #Planting_Nigeria <- data.frame(st=seq(1, 365, 7), en = (seq(1, 365, 7) + 454), weekNr = seq(1:53))
    # WLYCYData <- merge(wlydata, Planting_Nigeria[, c("st", "weekNr"), ], by.x="pl_Date", by.y="st")
...
getFRrecText <- function(ds, country, fertilizers, rootUP) {
  ...
  if (is.null(rec)) {
    ...
    #trans
    recom <- if (country == "NG" | country == "GH") {
      paste(norecom_ng)
    }else if (country == "TZ"){
      paste(norecom_tz)
    }else{
    ...
    if (rec$TC == 0) {
      ...
      # fertilizerTypes <- gsub("rate", "", names(ds)[grepl("rate", names(ds))])
      # fertilizerRates <- ds[,grepl("rate", names(ds))]
      # NR <- formatC(signif(ds$GR - ds$TC, digits=3), format="f", big.mark=",", digits=0)
      ...
      #adjNR <- ((rec$TargetY - rec$CurrentY) * rootUP) - AdJTC
      # NR <- formatC(signif(adjNR, digits=3), format="f", big.mark=",", digits=0)
    ...
    if (ds[1,]$CP) {
      ...
      if (ds[1,]$HD != ds[ds$CP == TRUE,]$HD) {
        ...
        if(country == "NG" | country == "GH"){
          ...
          recH <- paste0("Tarehe uliyochagua ya kuvuna ni bora, panda mihogo yako tarehe ",format(ds[1,]$HD, "%d %B %Y"), ".", sep=" ")
        }else{
...
getPPrecText <- function(ds, country = c("NG", "TZ", "RW")) {
  ...
  TRNS <- read.csv("translations_TEST.csv", stringsAsFactors = FALSE)
  no_ng <- gsub(pattern = "\"", replacement = "", TRNS$no[1]); no_tz <- gsub(pattern = "\"", replacement = "", TRNS$no[2]); no_rw <- gsub(pattern = "\"", replacement = "", TRNS$no[3])
  plo_ng <- gsub(pattern = "\"", replacement = "", TRNS$plo[1]); plo_tz <- gsub(pattern = "\"", replacement = "", TRNS$plo[2]); plo_rw <- gsub(pattern = "\"", replacement = "", TRNS$plo[3])
  ridg_ng <- gsub(pattern = "\"", replacement = "", TRNS$ridg[1]); ridg_tz <- gsub(pattern = "\"", replacement = "", TRNS$ridg[2]); ridg_rw <- gsub(pattern = "\"", replacement = "", TRNS$ridg[3])
  decnet_ng <- gsub(pattern = "\"", replacement = "", TRNS$decnet[1]); decnet_tz <- gsub(pattern = "\"", replacement = "", TRNS$decnet[2]);decnet_rw <- gsub(pattern = "\"", replacement = "", TRNS$decnet[3])
  werec_ng <- gsub(pattern = "\"", replacement = "", TRNS$werec[1]); werec_tz <- gsub(pattern = "\"", replacement = "", TRNS$werec[2]);werec_rw <- gsub(pattern = "\"", replacement = "", TRNS$werec[3]) 
  plofol_ng <- gsub(pattern = "\"", replacement = "", TRNS$plofol[1]); plofol_tz <- gsub(pattern = "\"", replacement = "", TRNS$plofol[2]);plofol_rw <- gsub(pattern = "\"", replacement = "", TRNS$plofol[3])
  noridg_ng <- gsub(pattern = "\"", replacement = "", TRNS$noridg[1]); noridg_tz <- gsub(pattern = "\"", replacement = "", TRNS$noridg[2]); noridg_rw <- gsub(pattern = "\"", replacement = "", TRNS$noridg[3])
  thank_ng <- gsub(pattern = "\"", replacement = "", TRNS$thank[1]); thank_tz <- gsub(pattern = "\"", replacement = "", TRNS$thank[2]); thank_rw <- gsub(pattern = "\"", replacement = "", TRNS$thank[3])
  plodir_ng <- gsub(pattern = "\"", replacement = "", TRNS$noplo[1]); plodir_tz <- gsub(pattern = "\"", replacement = "", TRNS$noplo[2]); plodir_rw <- gsub(pattern = "\"", replacement = "", TRNS$noplo[3])
  zerot_ng <- gsub(pattern = "\"", replacement = "", TRNS$zerot[1]); zerot_tz <- gsub(pattern = "\"", replacement = "", TRNS$zerot[2]); zerot_rw <- gsub(pattern = "\"", replacement = "", TRNS$zerot[3])
  changcost_ng <- gsub(pattern = "\"", replacement = "", TRNS$changcos[1]); changcost_tz <- gsub(pattern = "\"", replacement = "", TRNS$changcos[2]); changcost_rw <- gsub(pattern = "\"", replacement = "", TRNS$changcos[3])
  this_ng <- gsub(pattern = "\"", replacement = "", TRNS$this[1]); this_tz <- gsub(pattern = "\"", replacement = "", TRNS$this[2]); this_rw <- gsub(pattern = "\"", replacement = "", TRNS$this[3])
  decr_ng <- gsub(pattern = "\"", replacement = "", TRNS$decr[1]); decr_tz <- gsub(pattern = "\"", replacement = "", TRNS$decr[2]); decr_rw <- gsub(pattern = "\"", replacement = "", TRNS$decr[3])
  incr_ng <- gsub(pattern = "\"", replacement = "", TRNS$incr[1]); incr_tz <- gsub(pattern = "\"", replacement = "", TRNS$incr[2]); incr_rw <- gsub(pattern = "\"", replacement = "", TRNS$incr[3])
  costb_ng <- gsub(pattern = "\"", replacement = "", TRNS$costb[1]); costb_tz <- gsub(pattern = "\"", replacement = "", TRNS$costb[2]); costb_rw <- gsub(pattern = "\"", replacement = "", TRNS$costb[3])
  rtprod_ng <- gsub(pattern = "\"", replacement = "", TRNS$rtprod[1]); rtprod_tz <- gsub(pattern = "\"", replacement = "", TRNS$rtprod[2]); rtprod_rw <- gsub(pattern = "\"", replacement = "", TRNS$rtprod[3])
  optim_ng <- gsub(pattern = "\"", replacement = "", TRNS$optim[1]); optim_tz <- gsub(pattern = "\"", replacement = "", TRNS$optim[2]); optim_rw <- gsub(pattern = "\"", replacement = "", TRNS$optim[3]);
  ...
  #CHANGE
  ds$method_ploughing <- ifelse(country == "TZ" & ds$method_ploughing == "tractor", "kwa trekta", as.character(ds$method_ploughing))
  ds$method_ploughing <- ifelse(country == "TZ" & ds$method_ploughing == "manual", "kwa jembe la mkono", as.character(ds$method_ploughing))
  ds$method_ridging <- ifelse(country == "TZ" & ds$method_ridging == "tractor", "kwa trekta", as.character(ds$method_ridging))
  ds$method_ridging <- ifelse(country == "TZ" & ds$method_ridging == "manual", "kwa jembe la mkono", as.character(ds$method_ridging))
  ...
  if (ds[1,]$CP) {
    ...
    rec <- if (country == "NG") {
      ...
      paste0(optim_tz,
             ...
             ifelse(ds[1,]$method_ridging == "N/A", paste(no_tz), paste(ridg_tz), ds[1,]$method_ridging), paste(ridg_tz), "\n", " ",
             decnet_tz)
    }else{
      paste0(optim_rw,
    ...
    if (!ds[1,]$ploughing & ds[1,]$ridging) { recT <- if (country == "NG") {
      ...
      paste0(plodir_rw, ds[1,]$method_ridging, ridg_rw, "\n")
    }
    }
    ...
    rtcurr_ng <- gsub(pattern = "\"", replacement = "", TRNS$rtcurr[1]); rtcurr_tz <- gsub(pattern = "\"", replacement = "", TRNS$rtcurr[2]); rtcurr_rw <- gsub(pattern = "\"", replacement = "", TRNS$rtcurr[3])
    netinc_ng <- gsub(pattern = "\"", replacement = "", TRNS$netinc[1]); netinc_tz <- gsub(pattern = "\"", replacement = "", TRNS$netinc[2]); netinc_rw <- gsub(pattern = "\"", replacement = "", TRNS$netinc[3])
    ...
    tonb_ng <- gsub(pattern = "\"", replacement = "", TRNS$tonb[1]); tonb_tz <- gsub(pattern = "\"", replacement = "", TRNS$tonb[2]); tonb_rw <- gsub(pattern = "\"", replacement = "", TRNS$tonb[3])
    netno_ng <- gsub(pattern = "\"", replacement = "", TRNS$netno[1]); netno_tz <- gsub(pattern = "\"", replacement = "", TRNS$netno[2]); netno_rw <- gsub(pattern = "\"", replacement = "", TRNS$netno[3])
    incomp_ng <- gsub(pattern = "\"", replacement = "", TRNS$incomp[1]); incomp_tz <- gsub(pattern = "\"", replacement = "", TRNS$incomp[2]); incomp_rw <- gsub(pattern = "\"", replacement = "", TRNS$incomp[3])
    recap_ng <- gsub(pattern = "\"", replacement = "", TRNS$recap[1]); recap_tz <- gsub(pattern = "\"", replacement = "", TRNS$recap[2]); recap_rw <- gsub(pattern = "\"", replacement = "", TRNS$recap[3])
    kgof_ng <- gsub(pattern = "\"", replacement = "", TRNS$kgof[1]); kgof_tz <- gsub(pattern = "\"", replacement = "", TRNS$kgof[2]); kgof_rw <- gsub(pattern = "\"", replacement = "", TRNS$kgof[3])
    area_ng <- gsub(pattern = "\"", replacement = "", TRNS$area[1]); area_tz <- gsub(pattern = "\"", replacement = "", TRNS$area[2]); area_rw <- gsub(pattern = "\"", replacement = "", TRNS$area[3])
    willc_ng <- gsub(pattern = "\"", replacement = "", TRNS$willc[1]); willc_tz <- gsub(pattern = "\"", replacement = "", TRNS$willc[2]); willc_rw <- gsub(pattern = "\"", replacement = "", TRNS$willc[3])
    extrap_ng <- gsub(pattern = "\"", replacement = "", TRNS$extrap[1]); extrap_tz <- gsub(pattern = "\"", replacement = "", TRNS$extrap[2]); extrap_rw <- gsub(pattern = "\"", replacement = "", TRNS$extrap[3])
    tonof_ng <- gsub(pattern = "\"", replacement = "", TRNS$tonof[1]); tonof_tz <- gsub(pattern = "\"", replacement = "", TRNS$tonof[2]); tonof_rw <- gsub(pattern = "\"", replacement = "", TRNS$tonof[3])
    netincr_ng <- gsub(pattern = "\"", replacement = "", TRNS$netincr[1]); netincr_tz <- gsub(pattern = "\"", replacement = "", TRNS$netincr[2]); netincr_rw <- gsub(pattern = "\"", replacement = "", TRNS$netincr[3])
    ...
    if (dTC == 0) {
      ...
      recC <- if (country == "NG") {
        ...
        paste0(this_rw, ifelse(ds[1,]$dTC < 0, paste(decr_rw), paste(incr_rw)), costb_rw, currency, " ", dTC, ". ")
      }
    ...
    if (dRP == 0 & dNR > 0) { recP <- if (country == "NG") {
      ...
      paste0(rtcurr_rw, netinc_rw, currency, " ", dNR, ".")
    }
    }
    ...
    if (dRP != 0 & dNR == 0) { recP <- if (country == "NG") {
      ...
      paste0(rtwill_rw, ifelse(ds[1,]$TC < 0, paste(decr_rw), paste(incr_rw)), by_rw, dRP, tonb_rw,
             netno_rw)
    }
    }
    if (dRP != 0 & dNR != 0) { recP <- if (country == "NG") {
      ...
      paste0(rtwill_rw, ifelse(ds[1,]$TC < 0, paste(decr_rw), paste(incr_rw)), by_tz, dRP, tonb_rw, ifelse(ds[1,]$TC < 0, but_rw, and_rw),
             netinc_rw, currency, " ", dNR, incomp_rw)
    }
    }
