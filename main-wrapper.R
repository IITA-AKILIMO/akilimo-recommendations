...
#* @param SSPBagWt = 50
#* @param NPK201216available =  FALSE
#* @param NPK201216CostperBag = NA
#* @param NPK201216BagWt = 50
...
getRecommendation <- function(country = c("NG", "TZ"),
  ...
  print("Starting up production script processing")
  ...
  print(paste("FCY is",FCY))
  print(paste("Plumber processing started", Sys.time()))
  ...
  if (FR == TRUE) { res[["FR"]] <- getFRrecommendations(lat = lat,
    ...
    if (all(res$FR == "We do not have fertilizer recommendation for your location because your location is out of the recommendation domain AKILIMO is currently serving.")) {
      FRrecom <- FALSE
      recText[["FR"]] <- res$FR
    }else {
      if (res[["FR"]]$rec$NR > 0) {
        ...
        FRrecom <- FALSE
        recText[["FR"]] <- "Fertilizer application does not increase your revenue therefore it is not recommended for your field."
      }
    }
  ...
  if (email == TRUE) {
    print("Sending email")
    sendEmailReport(userEmail = userEmail, FR = FR, IC = IC, PP = PP, SP = SPH, country = country, FRrecom = FRrecom, ICrecom = ICrecom, PPrecom = PPrecom, SPrecom = SPrecom)
  }

  #print(paste("Returning recommendations", res, recText))
