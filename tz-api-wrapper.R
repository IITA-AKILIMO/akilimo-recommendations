...
getRecommendation <- function(country = c("NG", "TZ"),
    ...
    if(!is.na(HD_window) & !is.numeric(HD_window)) HD_window <- NA
    if(!is.na(PD_window) & !is.numeric(PD_window)) PD_window <- NA

    if(nameSF == "NA") nameSF <- NA


    # if(as.numeric(as.factor())) nameSF <- NA
    # if(as.numeric(as.factor(maxInv))) maxInv <- NA
    ...
    if(FR == TRUE)   {res[["FR"]] <- getFRrecommendations(lat = lat,
        ...
        if(all(res[["FR"]] != "We do not have fertilizer recommendation for your location.")){
            ...
            fertilizerAdviseTable(FR=FALSE, IC=TRUE, country)
        }else{
    ...
    if(IC == TRUE & country == "NG")                        {res[["IC"]] <- getICrecommendations(areaHa = areaHa,
                                                                                                 ...
                                                                                                 fertilizers = fertilizers,
                                                                                                 riskAtt = riskAtt)
        recText[["IC"]] <- getICrecText(ds = res$IC)
        ...
        IC_MarkdownText(rr = res$IC, fertilizers=fertilizers, userName=userName, country=country,
                        ...
                        maizePD=maizePD, rootUP = rootUP, cassPD = cassPD, maxInv = maxInv, CMP=CMP, maizeUP=maizeUP)
        fertilizerAdviseTable(FR=FALSE, IC=TRUE, country = "NG")}
    ...
    if(IC == TRUE & country == "TZ")                        {res[["IC"]] <- getCISrecommendations(areaHa = areaHa,
        ...
        CIS_MarkdownText(rr = res$IC, fertilizers=fertilizers, userName=userName, country=country,
                         ...
                         rootUP = rootUP, cassPD = cassPD, maxInv = maxInv, tuberUP=tuberUP)

        fertilizerAdviseTable(FR=FALSE, IC=TRUE, country = "TZ")}
    ...
    if(email == TRUE)                     {sendEmailReport(userEmail=userEmail, FR=FR, IC=IC, PP=PP, SP=SPH, country=country)}
